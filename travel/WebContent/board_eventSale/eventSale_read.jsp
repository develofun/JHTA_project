...
<div class="board_ldk" align="center">
	...
	<div class="board_ldk_tab">
		<ul>
			<li><a href="/eventSale.do?cmd=eventSale_read" class="clicked">전체</a></li>
			<li><a
				href="/eventSale.do?cmd=eventSale_search
						&eventSale_search_option=eventSale_sort
						&eventSale_search_text=이벤트">이벤트</a></li>
			<li><a
				href="/eventSale.do?cmd=eventSale_search
						&eventSale_search_option=eventSale_sort
						&eventSale_search_text=할인">할인</a></li>
			<li><a
				href="eventSale.do?cmd=eventSale_search
						&eventSale_search_option=eventSale_category
						&eventSale_search_text=의류">의류</a></li>
			<li><a
				href="eventSale.do?cmd=eventSale_search
						&eventSale_search_option=eventSale_category
						&eventSale_search_text=장비">장비</a></li>
			<li><a
				href="eventSale.do?cmd=eventSale_search
						&eventSale_search_option=eventSale_category
						&eventSale_search_text=숙박">숙박</a></li>
			<li><a
				href="eventSale.do?cmd=eventSale_search
						&eventSale_search_option=eventSale_category
						&eventSale_search_text=항공권">항공권</a></li>
			<li><a
				href="eventSale.do?cmd=eventSale_search
						&eventSale_search_option=eventSale_category
						&eventSale_search_text=패키지">패키지</a></li>
		</ul>
		<input type="button" id="btn_write" value="글쓰기"  style="display:none" 
			onclick="javascript:location.href='/eventSale.do?cmd=eventSale_create_page'">
	</div>
	...
	<div class="board_ldk_content">
		...
		<div class="board_ldk_paging">
			...
			<c:choose>
				<c:when test="${startPage>10 }">
					<input type="button" value=" ≪ "
						onclick="javascripte:location.href='/eventSale.do?cmd=eventSale_read&pageNum=${startPage-1 }&rowCount=${rowCount }'">
				</c:when>
				<c:otherwise>
					<input type="button" value=" ≪ ">
				</c:otherwise>
			...
			<c:choose>
				<c:when test="${pageNum>1 }">
					<input type="button" value=" 이전 "
						onclick="javascripte:location.href='/eventSale.do?cmd=eventSale_read&pageNum=${pageNum-1 }&rowCount=${rowCount }'">
				</c:when>
				<c:otherwise>
					<input type="button" value=" 이전 ">
				</c:otherwise>
			...
			<c:forEach var="i" begin="${startPage }" end="${endPage }">
				<c:choose>
					...
					<c:otherwise>
						<a
							href="/eventSale.do?cmd=eventSale_read&pageNum=${i }&rowCount=${rowCount }">[${i}]</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<c:choose>
				<c:when test="${pageCount>10 }">
					<input type="button" value=" ≫ "
						onclick="javascripte:location.href='/eventSale.do?cmd=eventSale_read&pageNum=${endPage+1 }&rowCount=${rowCount }'">
				</c:when>
				<c:otherwise>
					<input type="button" value=" ≫ ">
				</c:otherwise>
			...
			<div style="float: right">
				<select id="rowCount_change"
					onchange="location.href='/eventSale.do?cmd=eventSale_read&pageNum=${pageNum }&rowCount='+this.value">
		...
		<div class="board_ldk_search">
			<form action="/eventSale.do" method="post">
				<input type="hidden" name="cmd" value="eventSale_search"> <select
					name="eventSale_search_option">
					<option value="eventSale_writer">작성자</option>
					<option value="eventSale_title">제목</option>
					<option value="eventSale_content">내용</option>
				</select> <input type="text" name="eventSale_search_text"> <input
					type="submit" class="search_button" value="검색">
			</form>
		</div>
	</div>
</div>
