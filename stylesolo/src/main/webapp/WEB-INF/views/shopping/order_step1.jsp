...
<style type="text/css">
	...
	#outer{border:1px dotted gray; width:380px; height:110px;text-align: center;margin:5px;}
	#inner{backgroud-color:#eee; margin:10px}
	#inner ul{list-style: none;}
	#payment_{background-color: gray;border:1px dotted gray}
	#discountArea{margin-top: 15px;margin-bottom: 5px}
</style>
			...
			if(data.length>0){
				for(var i=0;i<data.length;i++){
					...
					for(var j=0;j<qty;j++){
						...
						"<div id='inner'>"+
							"<ul>"+
								"<li id='list_one'><h4>"+data[i].coupon_subject+"<h4></li>"+
								"<li id='list_two'><h4>"+data[i].coupon_discount+ "원</h4></li>"+ 
								"<li id='list_three'>(결제금액 "+data[i].coupon_value+"원 이상 시 사용가능)</li>"+
								"<li id='list_four'>"+data[i].coupon_startDate+"~"+data[i].coupon_endDate+"<span style='color:red'>"+using+"</span></li></ul></div></div>";	
...
function cp_load(){
	...
	function impPayment() {
		...
		IMP.request_pay({
			...
			name : '주문명:결제테스트',
			amount : 1000,
			buyer_email : 'dleorbs1985@naver.com',
			buyer_name : '구매자이름',
...
<div class="order_wrap">
	<form action="/payment.do" id="goPayment" onsubmit="return final_chk()">
		<div id="cart1">
			<div id="cart_top1">
		...
		<div id="order_item">
			<table style="border: 2px dotted #eee" id="items">
				...
				<tr>
					<td><input type="hidden" name="item_code" value="${vo.shop_item_code }">${vo.shop_item_code}</td>
					<td><img src="/resources/item_img/${vo.shop_item_mainimg_imgname}"></td>
					<td>${vo.shop_item_name}
						<c:if test="${options ne null }">
						<br>${vo.item_options_name }<input type="hidden" name="option_num" value="${vo.item_options_num }">
						</c:if>	
						<c:if test="${options eq null }">
							<input type="hidden" name="option_num" value="0">
						</c:if>
					</td>
					<td class="point"><input type="hidden" name="point" value="<fmt:formatNumber pattern="#">${Math.floor(vo.shop_item_pointrate * vo.shop_item_saleprice * vo.shop_cart_order_qty)}</fmt:formatNumber>"><span style="color:black"><fmt:formatNumber pattern="#">${Math.floor(vo.shop_item_pointrate * vo.shop_item_saleprice * vo.shop_cart_order_qty)}</fmt:formatNumber></span> p</td>
					<td><input type="hidden" name="saleprice" value="${vo.shop_item_saleprice }">${vo.shop_item_saleprice} 원</td>
					<td><input type="hidden" name="order_qty" value="${vo.shop_cart_order_qty}">${vo.shop_cart_order_qty}</td>
					<td class="total"><input type="hidden" name="total" value="${vo.shop_item_saleprice * vo.shop_cart_order_qty}"><span style="color:black">${vo.shop_item_saleprice * vo.shop_cart_order_qty}</span> 원</td>
				</tr>
				</c:forEach>
			</table>
