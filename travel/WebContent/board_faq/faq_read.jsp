...
<div class="board_ldk" align="center">	
	...
	<div class="board_ldk_tab">
		<ul>
			<li><a href="/faq.do?cmd=faq_read" class="clicked">전체</a></li>
			<li><a href="/faq.do?cmd=faq_search
						&faq_search_option=faq_category
						&faq_search_text=국내여행">국내여행</a></li>
			<li><a href="/faq.do?cmd=faq_search
						&faq_search_option=faq_category
						&faq_search_text=해외여행">해외여행</a></li>
			<li><a href="/faq.do?cmd=faq_search
						&faq_search_option=faq_category
						&faq_search_text=사이트 이용">사이트 이용</a></li>
			<li><a href="faq.do?cmd=faq_search
						&faq_search_option=faq_category
						&faq_search_text=로그인">로그인</a></li>
			<li><a href="faq.do?cmd=faq_search
						&faq_search_option=faq_category
						&faq_search_text=회원가입">회원가입</a></li>			
		</ul>
	...
	<div class="board_ldk_paging">
		...
		<c:choose>
			...
			<input type="button" value=" ≪ " onclick="javascripte:location.href='/faq.do?cmd=faq_read&pageNum=${startPage-1 }&rowCount=${rowCount }'">			
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			...
			<input type="button" value=" 이전 " onclick="javascripte:location.href='/faq.do?cmd=faq_read&pageNum=${pageNum-1 }&rowCount=${rowCount }'">
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:choose>
				...
				<c:otherwise><a href="/faq.do?cmd=faq_read&pageNum=${i }&rowCount=${rowCount }">[${i}]</a></c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			...
			<input type="button" value=" 다음 " onclick="javascripte:location.href='/faq.do?cmd=faq_read&pageNum=${pageNum+1 }&rowCount=${rowCount }'">
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			...
			<input type="button" value=" ≫ " onclick="javascripte:location.href='/faq.do?cmd=faq_read&pageNum=${endPage+1 }&rowCount=${rowCount }'">
			</c:when>
			<c:otherwise>			
			<input type="button" value=" ≫ ">
			</c:otherwise>
...
<script>
	...
	*/
	...
	function login_check(){
		...
		var update_link=document.getElementsByClassName("update_link");
		if("${sessionScope.member_nickname }"=="" || "${sessionScope.member_power}"=="agent"){
			btn_write.style.display="none";
			for(var i=0;i<update_link.length;i++){
				update_link[i].style.display="none";
			}			
		}
	}	
	login_check();
