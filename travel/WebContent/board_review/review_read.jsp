...
<div class="list_all">
	...
	<div class="board_ksw_tab">
		<ul>
			<li><a href="/review.do?cmd=review_read" class="clicked">전체</a></li>
			<li><a href="/review.do?cmd=review_search&review_search_option=review_category&review_search_text=국내">국내</a></li>
			<li><a href="/review.do?cmd=review_search&review_search_option=review_category&review_search_text=해외">해외</a></li>
		</ul>
	</div>
		...
		<table class="table_ksw">
		  ...
		  <c:forEach var="dto" items="${list }">
		  	<tr class="tr_ksw">
		  		...
		  		<td class="td_ksw">${dto.review_category }</td>
		  		<!-- 제목'...'처리 -->
		  		<td class="td_ksw">
		  		<c:choose>
           			<c:when test="${fn:length(dto.review_title) > 30}">
	  					<a href="/review.do?cmd=detail&review_num=${dto.review_num }"style="text-decoration:none">${fn:substring(dto.review_title,0,13)}....</a>
	  				</c:when>
	  				<c:otherwise>
            			<a href="/review.do?cmd=detail&review_num=${dto.review_num }"style="text-decoration:none">${dto.review_title }</a>
           			</c:otherwise> 
	  			</c:choose>
	  			</td>
	...
	<div class="board_ksw_paging">
		...
		<c:choose>
			...
			<input type="button" value=" ≪ " onclick="javascripte:location.href='/review.do?cmd=review_read&pageNum=${startPage-1 }&rowCount=${rowCount }'">			
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			...
			<input type="button" value=" 이전 " onclick="javascripte:location.href='/review.do?cmd=review_read&pageNum=${pageNum-1 }&rowCount=${rowCount }'">
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${startPage }" end="${endPage }">
			<c:choose>
				...
				<c:otherwise><a href="/review.do?cmd=review_read&pageNum=${i }&rowCount=${rowCount }">[${i}]</a></c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:choose>
			...
			<input type="button" value=" 다음 " onclick="javascripte:location.href='/review.do?cmd=review_read&pageNum=${pageNum+1 }&rowCount=${rowCount }'">
			</c:when>
			<c:otherwise>
		...
		<c:choose>
			...
			<input type="button" value=" ≫ " onclick="javascripte:location.href='/review.do?cmd=review_read&pageNum=${endPage+1 }&rowCount=${rowCount }'">
			</c:when>
			<c:otherwise>			
			<input type="button" value=" ≫ ">
			</c:otherwise>
		...
		<div style="float:right">
			<select id="rowCount_change" onchange="location.href='/review.do?cmd=review_read&pageNum=${pageNum }&rowCount='+this.value">
